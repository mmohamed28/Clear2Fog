...
def parse_arguments():
    ...
    parser.add_argument('-a', '--alphas', help='list of alpha values', type=float, nargs='+')
    ...
    parser.add_argument('-H', '--shift', help='toggle to incorporate shift', type=bool, default=True)
    parser.add_argument('-s', '--save_path', help='path to ',
                        default=str(Path(__file__).parent.absolute() / 'integral_lookup_tables'))
    ...
    if arguments.alphas is None:
        arguments.alphas = [0.005, 0.01, 0.02, 0.03, 0.06, 0.1, 0.12, 0.15, 0.2]
...
def generate_integral_lookup_tables(arguments) -> None:
    ...
    for alpha in arguments.alphas:
        ...
        filename = f'integral_0m_to_{r_0_max}m_stepsize_{granularity}m_tau_h_20ns_alpha_{alpha:.3f}.pickle'
