...
def build_net(ntype,nin,nwb=None,name=None):
    if ntype=='conv':
        return tf.nn.relu(tf.nn.conv2d(nin,filters=nwb[0],strides=[1,1,1,1],padding='SAME',name=name)+nwb[1])
    elif ntype=='pool':
...
def spp(net,channel=64,scope='g_pool'):
    ...
    net = tf.concat([
      tf.image.resize(net_2,(tf.shape(net)[1],tf.shape(net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_8,(tf.shape(net)[1],tf.shape(net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_16,(tf.shape(net)[1],tf.shape(net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_32,(tf.shape(net)[1],tf.shape(net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      net],axis=3)
...
def build_aggasatt_joint(input,channel=64,vgg_19_path='None'):
    ...
    for layer_id in range(1,6):
        ...
        input = tf.concat([tf.image.resize(vgg19_f,(tf.shape(input)[1],tf.shape(input)[2]), method=tf.image.ResizeMethod.BILINEAR)/255.0,input], axis=3)
...
def conv(batch_input, out_channels, stride):
    with tf.compat.v1.variable_scope("conv"):
        ...
        filter = tf.compat.v1.get_variable("filter", [4, 4, in_channels, out_channels], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(0, 0.02))
        padded_input = tf.pad(batch_input, [[0, 0], [1, 1], [1, 1], [0, 0]], mode="CONSTANT")
        conv = tf.nn.conv2d(padded_input, filters=filter, strides=[1, stride, stride, 1], padding="VALID")
        return conv

def lreluX(x, a):
    with tf.compat.v1.name_scope("lreluX"):
        x = tf.identity(x)
...
def batchnorm(input):
    with tf.compat.v1.variable_scope("batchnorm"):
        ...
        scale = tf.compat.v1.get_variable("scale", [channels], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(1.0, 0.02))
